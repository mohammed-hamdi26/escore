...
function AwardsForm({ awardsType, award, games, tournaments, id, t, setOpen }) {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      try {
        if (award) {
          await editAward(awardsType, id, awardData);
        } else {
          await addAward(awardsType, id, awardData);
        }
        formik.resetForm();
        toast.success(
          award
            ? t("Award updated successfully")
            : t("Award added successfully")
        );
        setOpen(false);
      } catch (error) {
        toast.error(
          error.message ||
            (award ? t("Error updating award") : t("Error adding award"))
        );
      }
    },
    validationSchema,
  ...
  const handleCropComplete = async (croppedBlob) => {
    ...
    try {
      const croppedFile = new File([croppedBlob], "award-image.png", {
        type: "image/png",
      });
  ...
  return (
    <form onSubmit={formik.handleSubmit} className="space-y-5">
      ...
      <InputApp
        ...
        placeholder={t("namePlaceholder") || "Enter award name"}
