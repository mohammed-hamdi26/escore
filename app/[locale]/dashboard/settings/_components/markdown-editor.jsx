...
import { Bold, Italic, List, ListOrdered, Heading1, Heading2, Link, Image, Code, Eye, FileDown, AlignLeft, AlignCenter, AlignRight, AlignJustify } from 'lucide-react';
...
export default function MarkdownEditor({ initialContent = null, contentId = null, mode = 'create' }) {
  ...
  const insertAlignment = (alignment) => {
    ...
    if (selectedText === '') {
      ...
      const lineEndInAfter = textAfterCursor.indexOf('\n');
      const lineEnd = lineEndInAfter === -1 ? content.length : start + lineEndInAfter;
      
      textToAlign = content.substring(lineStart, lineEnd);
      replaceStart = lineStart;
      replaceEnd = lineEnd;
    }
    
    // Wrap text with alignment div
    const alignedText = `<div align="${alignment}">${textToAlign}</div>`;
    const newText = content.substring(0, replaceStart) + alignedText + content.substring(replaceEnd);
    
    setContent(newText);
    setTimeout(() => {
      textarea.focus();
      const newPos = replaceStart + alignedText.length;
  ...
  const alignmentButtons = [
    { icon: AlignLeft, action: () => insertAlignment('left'), title: 'Align Left' },
    { icon: AlignCenter, action: () => insertAlignment('center'), title: 'Align Center' },
    { icon: AlignRight, action: () => insertAlignment('right'), title: 'Align Right' },
    { icon: AlignJustify, action: () => insertAlignment('justify'), title: 'Align Justify' },
  ];
  ...
  const renderMarkdown = (text) => {
    ...
    // Handle alignment divs (they're already HTML, so keep them as is)
    // Just process line breaks for non-div content
    html = html.replace(/\n/g, '<br />');
  ...
  return (
    <div className="w-full min-h-[80vh] flex flex-col bg-gray-50 text-stone-900">
      {isLoading && (
